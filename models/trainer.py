...
class Trainer():
    def __init__(self, model_wrapper: ModelWrapper, train_dataset: Dataset, valid_dataset: Dataset):
        ...
        self.train_data = DataLoader(dataset=train_dataset, batch_sampler=self.train_sampler,
                                      collate_fn=model_wrapper.collator.collate, num_workers=4, prefetch_factor=32, pin_memory=True)

        self.valid_data = DataLoader(dataset=valid_dataset, batch_sampler=self.valid_sampler,
                                      collate_fn=model_wrapper.collator.collate, num_workers=4, prefetch_factor=32, pin_memory=True)
        self.checkpoint_cycle = math.ceil((len(self.train_data) / CHECKPOINT_FREQ) / PRINT_PER_ITER) * PRINT_PER_ITER
        self.total_training_steps = int(len(self.train_dataset) * MAX_EPOCHS)
    ...
    def step(self, batch, training=True):
        ...
        if training:
            ...
            self.optimizer.step()
            self.scheduler.step(self.iter + self.current_epoch * len(self.train_dataset))
            return batch_loss
        else:
            self.model.eval()
    ...
    def train(self):
        ...
        for epoch_id in range(self.start_epoch, self.max_epochs):
            ...
            ## Reset sampler is important when we continue trainning from middle
            ## Validation before next epoch
            self.train_sampler.reset_sampler()
            torch.cuda.empty_cache()
            val_loss, val_accu, val_mean_time = self.validate()
    ...
    def load_checkpoint(self, checkpoint_dir, dataset_name, start_epoch=0):
        ...
        if start_epoch > 0 and os.path.exists(checkpoint_path):
            ...
            assert MAX_EPOCHS >= checkpoint['max_epoch']
            ...
            self.total_training_steps = int(len(self.train_dataset) * (MAX_EPOCHS - start_epoch))

            if checkpoint['strategy'] == "random_sampling":
                assert BUCKET_SAMPLING == False
                self.train_sampler.load_checkpoints(self.iter)
            else:
                assert BUCKET_SAMPLING == True
                self.train_sampler.load_checkpoints(self.remained_indies)

            self.total_training_steps += len(checkpoint['remained_indies'])
            self.logger.log(
            f"============TOTAL CONTINUE TRAINING STEPS===========\n{self.total_training_steps}")
    ...
    def save_checkpoint(self, checkpoint_dir, epoch, best_F1):
        ...
        state = {
            'epoch': epoch,
            'iter': self.iter, 'state_dict': self.model.state_dict(),
